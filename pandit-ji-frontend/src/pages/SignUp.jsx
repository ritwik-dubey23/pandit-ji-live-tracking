    ...
    const handleSimulatedGoogleSignUp = async () => {
        try {
            ...
            if (errStr.includes("API key not valid") || errStr.includes("API_KEY_INVALID") || errStr.includes("invalid-api-key") || error?.code === "auth/invalid-api-key") {
                ...
                setErr("Firebase Google Sign-In not enabled: Please enable Google Provider in Firebase Console -> Authentication -> Sign-in method -> Google. You can also sign up directly using Email & Password.");
                ...
                setErr(`Firebase URL Error (auth/invalid-continue-uri): The domain '${window.location.origin}' is not authorized in Firebase Console. Please add '${window.location.origin}' to Firebase Console -> Authentication -> Settings -> Authorized domains. You can also sign up directly using Email & Password.`);
            } else {
